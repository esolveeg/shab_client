    ...
    <v-container>
      <div class="event-wrapper">
        ...
        <div class="left">
          ...
          <div class="subscribe" @click.prevent="openModal">
            <div class="price">
              {{ price(event.Price) }}
            </div>
        ...
        <v-stepper-content step="2">
          <v-card>
            <v-card-title v-if="roles != null">
              قم بعمل تحويل بمبلع {{ price(roles[form.Role_id - 1].Price) }} الي
              هذا الحساب البنكي و سيتم التواصل معك لتاكيد العضوية
            </v-card-title>
            <v-card-text>
              <ul>
...
export default {
    ...
    finished: {
      get() {
        let eventDate = new Date(
          `${this.event.Year}-${this.event.Month}-${this.event.Day}`
        )
        let today = new Date()
  ...
  mounted() {
    ...
    Event(this.$route.params.id).then((val) => {
      console.log(val)
      this.event = val
      this.loading = false
    })
